...
- 当有人调用 push_back 函数时，会先获取互斥锁 mtx\_，以确保在进行操作时其他线程无法同时访问队列 deq\_。
- 如果队列已满（deq\_.size() >= capacity\_），则通过条件变量 condProducer\_ 进入等待状态，等待消费者线程通知队列中有空间可用。
- 当队列有空间可用时，加锁，将数据 item 放入队列 deq\_ 中，并通过条件变量 condConsumer\_ 通知消费者线程有数据可取。
- 最后，释放互斥锁 mtx\_，允许其他线程访问队列。

**pop 函数**

- 当有人调用 pop 函数时，也会先获取互斥锁 mtx\_，以确保在进行操作时其他线程无法同时访问队列 deq\_。
- 如果队列为空（deq\_.empty()），则通过条件变量 condConsumer\_ 进入等待状态，等待生产者线程通知队列中有数据可取。在等待期间，如果 isClose\_ 为 true，则表示队列已关闭，直接返回 false。
- 当队列中有数据可取时，加锁，将队列头部的数据赋值给 item，然后从队列中删除该数据，并通过条件变量 condProducer\_ 通知生产者线程有空间可用。
- 最后，释放互斥锁 mtx\_，允许其他线程访问队列。

> :question:为什么判断线程执行的条件用 while 而不是 if？
>
...
在 Linux 内核中，当网络数据到达时，主要经过以下过程：

1. **网络协议栈处理**：当网络数据到达网络接口时，Linux 内核会将数据送入网络协议栈。这个过程包括数据链路层（如以太网）、网络层（如 IP 层）、传输层（如 TCP 或 UDP 层）等一系列处理步骤。
2. **TCP 协议栈处理**：如果数据是通过 TCP 协议传输的，内核会将数据交给 TCP 协议栈进行处理。TCP 协议栈负责处理数据的可靠传输、流量控制、拥塞控制等。
3. **套接字处理**：当 TCP 协议栈接收到数据后，它会根据套接字（socket）的标识符找到对应的接收缓冲区。每个套接字都有一个接收缓冲区，用于存储接收到的数据。
4. **复制数据到用户空间**：一旦数据到达接收缓冲区，内核会将数据从内核空间复制到应用程序的用户空间。这个过程通常是通过系统调用来完成的，例如 `recv()` 函数。
5. **通知应用程序**：一旦数据复制完成，内核会通知应用程序数据已经可用，应用程序可以通过系统调用得到通知，然后读取数据并进行处理。
